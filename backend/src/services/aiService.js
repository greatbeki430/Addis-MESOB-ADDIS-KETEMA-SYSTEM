...
const callGemini = async (
  ...
  try {
    ...
    const geminiFinish = response.candidates?.[0]?.finishReason;
    if (geminiFinish === "MAX_TOKENS") {
      console.warn("[aiService] ⚠️ Gemini response truncated by max_tokens");
    }
    return {
      text: response.text,
      provider: PROVIDERS.GEMINI,
      truncated: geminiFinish === "MAX_TOKENS",
    };
  } catch (err) {
...
const callGroq = async (
  ...
  try {
    ...
    const groqChoice = response.choices[0];
    if (groqChoice?.finish_reason === "length") {
      console.warn("[aiService] ⚠️ Groq response truncated by max_tokens");
    }
    return {
      text: groqChoice?.message?.content || "",
      provider: PROVIDERS.GROQ,
      truncated: groqChoice?.finish_reason === "length",
    };
  } catch (err) {
...
const callCohere = async (
  ...
  try {
    ...
    if (response?.finishReason === "MAX_TOKENS") {
      console.warn("[aiService] ⚠️ Cohere response truncated by max_tokens");
    }
    return {
      text,
...
const callDeepSeek = async (
  ...
  try {
    ...
    const dsChoice = data?.choices?.[0];
    const text = dsChoice?.message?.content || "";
    if (dsChoice?.finish_reason === "length") {
      console.warn("[aiService] ⚠️ DeepSeek response truncated by max_tokens");
    }
    return {
      text,
      provider: PROVIDERS.DEEPSEEK,
      truncated: dsChoice?.finish_reason === "length",
    };
  } catch (err) {
...
const generateText = async (
  ...
  for (const provider of callers) {
    ...
    try {
      ...
      if (result.truncated) {
        console.warn(
          `[aiService] ⚠️ Final response from ${result.provider} was truncated by max_tokens`,
        );
      }
      generateText.lastProvider = result.provider;
      generateText.lastTruncated = !!result.truncated;
      return result.text;
    } catch (err) {
      console.warn(
...
const generateEvaluationSummary = async (evaluationData) => {
  ...
  // ✅ ENHANCED: More explicit instruction for Amharic to prevent truncation
  const targetLanguage = language === "en" ? "en" : "am";

  let languageInstruction;
  if (targetLanguage === "am") {
    languageInstruction = `
    ⚠️ CRITICAL INSTRUCTION FOR AMHARIC OUTPUT:
    - You MUST write the ENTIRE response in formal, professional Amharic (አማርኛ) using proper Ethiopic script.
    - The response MUST be COMPLETE with all 3 required paragraphs fully written.
    - Only people's names and department names may stay in their original form.
    - Do NOT switch to English mid-response.
    - Write complete, well-developed paragraphs. Do not pad for length —
      prioritize finishing all three sections cleanly over hitting a word count.
    - Write in a professional government report tone.
    - Do NOT mention that this was generated by AI.
    - Do NOT refer to yourself as an AI or assistant.
    `;
  } else {
    languageInstruction = `
    Write the entire narrative in English.
    - Professional government report tone.
    - Do NOT mention that this was generated by AI.
    - Do NOT refer to yourself as an AI or assistant.
    `;
  }
...
Write a complete professional performance evaluation narrative:
...
BEST PERFORMER:
- Name: ${topScorer?.member || topScorer?.name || "N/A"}
- Score: ${topScorer?.total || "N/A"} points
...
Write 3 complete paragraphs (not bullet points):
