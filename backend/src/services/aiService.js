...
const generateEvaluationSummary = async (evaluationData) => {
  ...
  // Evaluation report PDFs in this system are Amharic-only (see
  // ReportExport.jsx's forced REPORT_LANG and pdfExport.js's Amharic
  // labels), so the narrative should default to Amharic too — otherwise
  // it silently comes back in English and gets embedded in an otherwise
  // all-Amharic report. Callers can still request English explicitly via
  // evaluationData.language === "en".
  const targetLanguage = language === "en" ? "en" : "am";
  const languageInstruction =
    targetLanguage === "am"
      ? "Write the ENTIRE narrative in formal, professional Amharic (አማርኛ) using proper Ethiopic script. Do not switch into English — only people's names and department names may stay as given."
      : "Write the entire narrative in English.";

  const prompt = `${languageInstruction}

Write a professional performance evaluation narrative:
Team: ${teamName}
...
Under 200 words total. Professional government report tone. Write it as a
normal report section — do not mention that it was generated by AI, and do
not refer to yourself as an AI or assistant anywhere in the output.`;
